        ...
        <div class="container-fluid"><a class="navbar-brand" href="./" style="font-family: Montserrat, sans-serif;color: rgba(255,255,255,0.9);">SJC Ents</a><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
            <div
                ...
                <ul class="nav navbar-nav ml-auto">
                    ...
                    <?php echo '<li class="nav-item"><a class="nav-link active" href="https://raven.cam.ac.uk/auth/logout.html" style="color: rgba(255,255,255,0.9);font-family: Montserrat, sans-serif;">Logout '.$_SERVER['REMOTE_USER'].'</a></li>' ?>
                </ul>
        </div>
        </div>
    </nav>
<header>    
    ...
    $sql = "SELECT * FROM hlb_yeargroup WHERE yg_crsid = '".$_SERVER['REMOTE_USER']."'";
    $res = $con_sbr->query($sql);
    $yg_content = 'B';
    foreach($res as $val){
        $yg_content = $val['yg_yeargroup'];
    }

    if($yg_content == 'I'){
        $sql = "SELECT * FROM hlb_events WHERE NOW() < event_end_datetime AND event_vis = 1 ORDER BY event_start_datetime"; 
    }else{
        $sql = "SELECT * FROM hlb_events WHERE NOW() < event_end_datetime AND event_vis = 1 AND (event_yeargroup = '".$yg_content."' OR event_yeargroup = 'B') ORDER BY event_start_datetime"; 
    }
    ...
    <main>
        <div class="container">
            <section class="clean-block about-us" style="padding: 0px 0px;">
                <h2 class="text-center text-black-50" style="padding: 25px 10px;/*background-image: linear-gradient(to bottom , #ececec, white);*/font-family: Montserrat, sans-serif;">Welcome to St John's College Ents!</h2>
                <?php 
                if($yg_content=='B'){
                    ?>
                    <p style="font-family: Montserrat, sans-serif;"> In order to see personalised content, please click the appropriate link...</p>
                    <a style="font-family: Montserrat, sans-serif;" href="reg?yg=UG"> I'm a undergraduate student.</a><br>
                    <a style="font-family: Montserrat, sans-serif;" href="reg?yg=I"> I'm somewhere in the middle (Part III etc.)</a><br>
                    <a style="font-family: Montserrat, sans-serif;" href="reg?yg=PG"> I'm a postgraduate student.</a><br>
