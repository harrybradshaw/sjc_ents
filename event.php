...
<header>
    ...
    <?php
        ...
        if($_GET['id']){
            $sql = "SELECT * FROM hlb_events WHERE event_id = ".$_GET['id']." ORDER BY event_start_datetime"; 
            $res = $con_sbr->query($sql);
            foreach($res as $val){
                ...
                <?php if($val['event_imgsrc']){
                   echo '<div class="card" style="border-style: none;"><img class="img-fluid card-img-top w-100 d-block border rounded" src="'.$val['event_imgsrc'].'" style="box-shadow: 0px 0px 10px 0px;">';
                    ...
                    <?php if($val['event_fblink']){
                        echo '<button class="btn btn-primary" type="button" style="background: rgb(204,31,31);" onclick="location.href=\''.$val['event_fblink'].'\'">Facebook Event</button>';
                        if($val['event_zoomlink']){
                            echo '&nbsp;&nbsp;&nbsp;';
                        }
                    }
                    if($val['event_zoomlink']){
                        echo '<button class="btn btn-primary" type="button" style="background: rgb(204,31,31);" onclick="location.href=\''.$val['event_fblink'].'\'">Zoom Link</button>';
                    } 
                    ?>
                    </div>
                    </div>
            ...
            <section class="text-left">
                ...
                if($val['event_vis']){
                    ...
                    foreach($res_b as $val_b){
                        ...
                        }elseif($num >= $val_b['slots_max']){
                            echo '<button type="submit" class="cancelbtn">Event Full</button>';
                        }else{
                            echo '<form class="modal-content" method="POST" action="form_send.php">';
                            echo '<input type="hidden" name="send_ref" value="event_book">';
                            echo '<input type="hidden" name="event_id" value="'.$val_b['event_id'].'">';
